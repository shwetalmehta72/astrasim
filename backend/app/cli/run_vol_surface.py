...
async def _run(symbol: str, target_date: date | None, force: bool) -> None:
    surface = await compute_surface(symbol, target_date, force=force)
    logger.info(
...
def main() -> None:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Force refresh even if cache/policies allow reuse",
    )
    args = parser.parse_args()
    parsed_date = date.fromisoformat(args.target_date) if args.target_date else None
    asyncio.run(_run(args.symbol, parsed_date, args.force))
